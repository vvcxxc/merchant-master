...
export default function Item({ item, onChange }: Props) {
  ...
  return (
    <Flex className={styles.item} justify='between' align='start' onClick={choose.bind(this, item)}>
      <Flex className={styles.item_left}>
        ...
        <Flex className={styles.item_info} direction='column' align='start' justify='between'>
          ...
          <div>已选{item.occupation_number ? item.occupation_number : 0}份，剩余{item.total_surplus_num}份</div>
      ...
      {
        item.is_choose ? <img src={require('@/assets/checked.png')} alt='' className={styles.checkout_icon} />
